...
public class OrchestrationConfig {
    ...
    @Bean
    CiObserver ciObserver(GitHubClient gitHubClient,
                          @Value("${devflowai.ci.poll-interval-seconds:5}") long pollIntervalSeconds,
                          @Value("${devflowai.ci.timeout-minutes:15}") long timeoutMinutes) {
        return new PollingCiObserver(gitHubClient, Duration.ofSeconds(pollIntervalSeconds),
                Duration.ofMinutes(timeoutMinutes));
    }

    @Bean
    ReleaseDispatcher releaseDispatcher(GitHubClient gitHubClient,
                                        @Value("${devflowai.release.workflow:}") String workflow,
                                        @Value("${devflowai.release.ref:main}") String ref) {
        return new GitHubActionsReleaseDispatcher(gitHubClient, workflow, ref);
    }

    @Bean
    ReleaseObserver releaseObserver(GitHubClient gitHubClient,
                                    @Value("${devflowai.release.poll-interval-seconds:5}") long pollIntervalSeconds,
                                    @Value("${devflowai.release.verification-timeout-minutes:20}") long timeoutMinutes) {
        return new PollingReleaseObserver(gitHubClient, Duration.ofSeconds(pollIntervalSeconds),
                Duration.ofMinutes(timeoutMinutes));
    }
